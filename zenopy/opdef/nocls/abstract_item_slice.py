...
from zenopy.builder.value import Value, ListValue, TupleValue, IntegerValue, NoneValue
...
class AbstractItemSliceOp(AbstractOp):
    ...
    def insert_slicing_number_assertion(self, number: IntegerValue, dim: int, reducer: AbsIRBuilderInterface):
        is_neg = reducer.ir_less_than_i(number, reducer.ir_constant_int(0))
        number = reducer.ir_add_i(number, reducer.ir_mul_i(reducer.ir_constant_int(dim), is_neg))
        is_out_of_range = reducer.ir_logical_or(
