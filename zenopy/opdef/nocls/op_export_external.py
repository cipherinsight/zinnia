...
from zenopy.builder.value import Value, IntegerValue, FloatValue, NDArrayValue, TupleValue, NoneValue, ListValue
...
class ExportExternalOp(AbstractOp):
    ...
    def build(self, reducer: AbsIRBuilderInterface, kwargs: Dict[str, Value], dbg: Optional[DebugInfo] = None) -> Value:
        ...
        if isinstance(x, IntegerValue):
            return reducer.ir_export_external_i(x, self.for_which, self.key, self.indices)
        elif isinstance(x, FloatValue):
            return reducer.ir_export_external_f(x, self.for_which, self.key, self.indices)
