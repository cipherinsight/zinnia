...
class SinHOp(AbstractOp):
    ...
    def build(self, reducer: AbsIRBuilderInterface, kwargs: Dict[str, Value], dbg: Optional[DebugInfo] = None) -> Value:
        ...
        if isinstance(x, IntegerValue):
            return reducer.ir_sinh_f(reducer.ir_float_cast(x))
        elif isinstance(x, FloatValue):
            return reducer.ir_sinh_f(x)
        elif isinstance(x, NDArrayValue) and isinstance(x.dtype(), IntegerDTDescriptor):
            return x.unary(FloatType, lambda u: reducer.ir_sinh_f(reducer.ir_float_cast(u)))
        elif isinstance(x, NDArrayValue) and isinstance(x.dtype(), FloatDTDescriptor):
            return x.unary(FloatType, lambda u: reducer.ir_sinh_f(u))
