# Source: NPBench polybench/deriche (deriche_numpy.py)
# Original signature: kernel(alpha, imgIn) where imgIn is (W, H).
# Migration notes:
#   - W, H hoisted as module-level shape constants.
#   - alpha kept as a float scalar parameter (data value, not shape).
#   - imgIn.shape[0]/[1] replaced with W / H to make loop bounds static.
from zinnia import *
...
@zk_circuit
def deriche(alpha: float, imgIn: NDArray[Float, 16, 16]):
    ...
    y2[-1, :] = 0.0
    y2[-2, :] = a7 * imgOut[-1, :]
    for i in range(16 - 3, -1, -1):
        y2[i, :] = (a7 * imgOut[i + 1, :] + a8 * imgOut[i + 2, :] +
                    b1 * y2[i + 1, :] + b2 * y2[i + 2, :])

    imgOut[:] = c2 * (y1 + y2)

    _zinnia_result = imgOut
