...
# Migration notes:
#   - xn, yn, itermax hoisted to module-level constants; values from the "S" preset (XN=YN=200) shrunk to 16; itermax=8.
...
def mandelbrot2(xmin: float, xmax: float, ymin: float, ymax: float,
    ...
    Xi, Yi = np.mgrid[0:16, 0:16]
    X = np.linspace(xmin, xmax, 16, dtype=np.float64)[Xi]
    Y = np.linspace(ymin, ymax, 16, dtype=np.float64)[Yi]
    C = X + Y * 1j
    N_ = np.zeros(C.shape, dtype=np.int64)
    Z_ = np.zeros(C.shape, dtype=np.complex128)
    Xi.shape = Yi.shape = C.shape = 16 * 16
