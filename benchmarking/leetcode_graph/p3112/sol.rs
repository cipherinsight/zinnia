...
fn verify_solution<F: ScalarField>(
    ...
    for i in 0..100 {
        let mut value = ctx.load_witness(F::from_u128((input.graph[i]).abs() as u128));
        if input.graph[i] < 0 {
            value = GateInstructions::neg(&gate, ctx, value);
        }
        graph.push(value);
    }
    // load disappear
    ...
    for i in 0..10 {
        let mut value = ctx.load_witness(F::from_u128((input.disappear[i]).abs() as u128));
        if input.disappear[i] < 0 {
            value = GateInstructions::neg(&gate, ctx, value);
        }
