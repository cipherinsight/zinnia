        ...
        if axes is None:
            ...
            for tuple_val in tuple_vals:
                if tuple_val is None:
                    raise StaticInferenceError(spi, f"Each element in `axes` should be able to be statically inferred")
            axes_vals = axes.get()
        elif isinstance(axes, NDArrayInferenceDescriptor):
            ndarray_vals = axes.get()
            if len(ndarray_vals.shape) != 1:
                raise TypeInferenceError(spi, "Invalid provided `NDArray` on `axes`, the shape of this `NDArray` should be exactly 1-dimension")
            for ele_val in ndarray_vals.values:
                if ele_val is None:
                    raise StaticInferenceError(spi, f"Each element in `axes` should be able to be statically inferred")
            axes_vals = axes.get().values
        else:
            raise TypeInferenceError(spi, "Invalid type on `axes`")
        permutation = [_ for _ in range(len(the_self.shape()))]
        for axe in axes_vals:
            if not 0 <= axe < len(the_self.shape()):
                raise TypeInferenceError(spi, f"Invalid axis value `{axe}`")
            if permutation[axe] is None:
                raise TypeInferenceError(spi, f"`axes` should be a permutation of 0 to {len(the_self.shape()) - 1}")
