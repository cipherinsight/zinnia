...
class NDArray_ArgMaxOp(AbstractAggregator):
    ...
    def aggregator_func(self, lhs: Any, lhs_i: int, rhs: Any, rhs_i: int, dt: DTDescriptor) -> Tuple[Any, int | None]:
        return max(lhs, rhs) if lhs is not None and rhs is not None else None, (rhs_i if rhs > lhs else lhs_i) if lhs is not None and rhs is not None else None
    ...
    def aggregator_build_ir(self, ir_builder, lhs: int, lhs_i: int, rhs: int, rhs_i: int, dt: DTDescriptor) -> Tuple[int, int | None]:
        if isinstance(dt, IntegerDTDescriptor):
            ...
            candidate_i = ir_builder.create_add_i(
                ir_builder.create_mul_i(cond, ir_builder.create_constant(rhs_i)),
                ir_builder.create_mul_i(not_cond, lhs_i)
            )
