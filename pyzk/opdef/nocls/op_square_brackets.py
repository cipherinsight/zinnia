...
class SquareBracketsOp(AbstractOp):
    ...
    def type_check(self, dbg_i: Optional[DebugInfo], kwargs: Dict[str, InferenceDescriptor]) -> DTDescriptor:
        ...
        if all([isinstance(arg.type(), IntegerDTDescriptor) for arg in args]):
            return NDArrayDTDescriptor((len(args), ), IntegerDTDescriptor())
        if all([isinstance(arg.type(), FloatDTDescriptor) for arg in args]):
            return NDArrayDTDescriptor((len(args), ), FloatDTDescriptor())
        if not all([isinstance(arg.type(), NDArrayDTDescriptor) for arg in args]):
            raise TypeInferenceError(dbg_i, "You can only create NDArray using square brackets on `Integer`, `Float` or `NDArray` types")
        for i, arg in enumerate(args[1:]):
            if arg.dt != args[i - 1].dt:
                raise TypeInferenceError(dbg_i,f"Create NDArray using square brackets failed because of elements shape or dtype mismatch: {arg.dt} != {args[i - 1].dt}")
        return NDArrayDTDescriptor((len(args), ) + args[0].shape(), args[0].dtype())

    def static_infer(self, dbg_i: Optional[DebugInfo], kwargs: Dict[str, InferenceDescriptor]) -> InferenceDescriptor:
        args = list(kwargs.values())
        if all([isinstance(arg.type(), IntegerDTDescriptor) for arg in args]):
            return NDArrayInferenceDescriptor((len(args), ), IntegerDTDescriptor(), NDArrayHelper((len(args), ), [arg.get() for arg in args]))
        if all([isinstance(arg.type(), FloatDTDescriptor) for arg in args]):
    ...
    def ir_flatten(self, ir_builder, kwargs: Dict[str, FlattenDescriptor]) -> FlattenDescriptor:
        args = list(kwargs.values())
        if all([isinstance(arg.type(), IntegerDTDescriptor) for arg in args]):
            return NDArrayFlattenDescriptor((len(args), ), IntegerDTDescriptor(), NDArrayHelper((len(args), ), [arg.ptr() for arg in args]))
        if all([isinstance(arg.type(), FloatDTDescriptor) for arg in args]):
