...
class TanOp(AbstractOp):
    ...
    def ir_flatten(self, ir_builder, kwargs: Dict[str, FlattenDescriptor]) -> FlattenDescriptor:
        ...
        if isinstance(x, IntegerFlattenDescriptor):
            return FloatFlattenDescriptor(ir_builder.create_tan(ir_builder.create_float_cast(x.ptr())))
        elif isinstance(x, FloatFlattenDescriptor):
            return FloatFlattenDescriptor(ir_builder.create_tan(x.ptr()))
        if isinstance(x, NDArrayFlattenDescriptor):
            if isinstance(x.dtype(), IntegerDTDescriptor):
                return NDArrayFlattenDescriptor(x.shape(), FloatDTDescriptor(), x.ptr().unary(lambda u: ir_builder.create_tan(ir_builder.create_float_cast(u))))
            elif isinstance(x.dtype(), FloatDTDescriptor):
                return NDArrayFlattenDescriptor(x.shape(), FloatDTDescriptor(), x.ptr().unary(lambda u: ir_builder.create_tan(u)))
        raise NotImplementedError()
