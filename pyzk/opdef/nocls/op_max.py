...
class MaxOp(AbstractOp):
    ...
    def type_check(self, dbg_i: Optional[DebugInfo], kwargs: Dict[str, InferenceDescriptor]) -> DTDescriptor:
        ...
        if len(args) == 1:
            ...
            if has_ndarray:
                raise TypeInferenceError(dbg_i, f"Should not pass NDArray as argument here. To perform element-wise {self.get_name()}, use maximum instead")
            if has_tuple:
        ...
        if len(args) == 1:
                ...
                if n0.dtype() == IntegerDTDescriptor():
                    for v in n0.get().values[1:]:
                        ...
                        candidate = ir_builder.create_add_i(
                            ir_builder.create_mul_i(cond, v),
                            ir_builder.create_mul_i(not_cond, candidate)
                        )
                ...
                if n0.dtype() == FloatDTDescriptor():
                    for v in n0.get().values[1:]:
                        ...
                        candidate = ir_builder.create_add_f(
                            ir_builder.create_mul_f(ir_builder.create_float_cast(cond), v),
                            ir_builder.create_mul_f(ir_builder.create_float_cast(not_cond), candidate)
                        )
            ...
            if args[0].dt == IntegerDTDescriptor():
                for v in args[1:]:
                    ...
                    not_cond = ir_builder.create_logical_not(cond)
                    candidate = ir_builder.create_add_i(
                        ir_builder.create_mul_i(cond, v.ptr()),
                        ir_builder.create_mul_i(not_cond, candidate)
                    )
                return IntegerFlattenDescriptor(candidate)
            if args[0].dt == FloatDTDescriptor():
