...
class PowIOp(AbstractOp):
    ...
    def static_infer(self, dbg_i: Optional[DebugInfo], kwargs: Dict[str, InferenceDescriptor]) -> InferenceDescriptor:
        ...
        if isinstance(lhs, IntegerInferenceDescriptor) and isinstance(rhs, IntegerInferenceDescriptor):
            if rhs.get() is not None and rhs.get() < 0:
                raise StaticInferenceError(dbg_i, f"Exponent must be non-negative on {self.get_name()}")
    ...
    def ir_flatten(self, ir_builder, kwargs: Dict[str, FlattenDescriptor]) -> FlattenDescriptor:
        ...
        if isinstance(lhs, IntegerFlattenDescriptor) and isinstance(rhs, IntegerFlattenDescriptor):
            return IntegerFlattenDescriptor(ir_builder.create_pow_i(lhs.ptr(), rhs.ptr()))
        raise NotImplementedError()
