...
from zinnia.compile.builder.value import IntegerValue, NDArrayValue, FloatValue, Value
...
class ExpOp(AbstractOp):
    ...
    def build(self, builder: AbsIRBuilderInterface, kwargs: Dict[str, Value], dbg: Optional[DebugInfo] = None) -> Value:
        ...
        if isinstance(x, IntegerValue):
            return builder.ir_exp_f(builder.ir_float_cast(x))
        elif isinstance(x, FloatValue):
            return builder.ir_exp_f(x)
        elif isinstance(x, NDArrayValue) and x.dtype() == IntegerType:
            return x.unary(IntegerType, lambda v: builder.ir_exp_f(builder.ir_float_cast(v)))
        elif isinstance(x, NDArrayValue) and x.dtype() == FloatType:
            return x.unary(IntegerType, lambda v: builder.ir_exp_f(v))
