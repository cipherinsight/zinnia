...
from zinnia.compile.builder.value import Value, NumberValue, NDArrayValue, IntegerValue, FloatValue
...
class PowOp(AbstractOp):
    ...
    def _build_pow(self, builder: AbsIRBuilderInterface, x: NumberValue, exponent: NumberValue) -> NumberValue:
        if isinstance(x, IntegerValue) and isinstance(exponent, IntegerValue):
            ...
            return builder.ir_pow_f(x, builder.ir_float_cast(exponent))
        raise NotImplementedError()
    ...
    def build(self, builder: AbsIRBuilderInterface, kwargs: Dict[str, Value], dbg: Optional[DebugInfo] = None) -> Value:
        ...
        result = NDArrayValue.binary(x_ndarray, exponent_ndarray, self._get_expected_result_dtype(x_dtype, exponent_dtype), lambda u, v: self._build_pow(builder, u, v))
        if mod is not None:
            ...
            result = NDArrayValue.binary(result, mod_ndarray, IntegerType, lambda u, v: builder.op_modulo(u, v))
