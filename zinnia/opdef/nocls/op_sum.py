...
class SumOp(AbstractOp):
    ...
    def build(self, builder: AbsIRBuilderInterface, kwargs: Dict[str, Value], dbg: Optional[DebugInfo] = None) -> Value:
        ...
        if isinstance(iterable, NDArrayValue):
            result = builder.op_get_item(iterable, builder.op_square_brackets([builder.op_parenthesis([builder.ir_constant_int(0)])]), dbg)
            for i in range(1, iterable.shape()[0]):
                result = builder.op_add(result, builder.op_get_item(iterable, builder.op_square_brackets([builder.op_parenthesis([builder.ir_constant_int(i)])]), dbg), dbg)
