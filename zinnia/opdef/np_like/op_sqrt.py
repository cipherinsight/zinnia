...
class NP_SqrtOp(AbstractOp):
    ...
    def build(self, builder: AbsIRBuilderInterface, kwargs: Dict[str, Value], dbg: Optional[DebugInfo] = None) -> Value:
        ...
        if isinstance(x, IntegerValue):
            return builder.ir_sqrt(builder.ir_float_cast(x))
        elif isinstance(x, FloatValue):
            return builder.ir_sqrt(x)
        elif isinstance(x, NDArrayValue) and isinstance(x.dtype(), IntegerDTDescriptor):
            return x.unary(FloatType, lambda u: builder.ir_sqrt(builder.ir_float_cast(u)))
        elif isinstance(x, NDArrayValue) and isinstance(x.dtype(), FloatDTDescriptor):
            return x.unary(FloatType, lambda u: builder.ir_sqrt(u))
