...
class GreaterThanOrEqualFIR(AbstractIR):
    ...
    def mock_exec(self, kwargs: Dict[str, Any], config: MockExecConfig) -> Any:
        return 1 if (kwargs["lhs"] > kwargs["rhs"] or abs(kwargs["lhs"] - kwargs["rhs"]) < config.float_tolerance()) else 0
    ...
    @staticmethod
    def import_from(data: Dict) -> 'GreaterThanOrEqualFIR':
        return GreaterThanOrEqualFIR()
