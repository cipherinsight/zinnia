...
from zinnia.compile.type_sys import DTDescriptor, FloatDTDescriptor, FloatType, IntegerType
...
class MatMulOp(AbstractOp):
    ...
    def _build_constant_zero(self, builder: AbsIRBuilderInterface, expected_dt: DTDescriptor):
    ...
    def build(self, builder: AbsIRBuilderInterface, kwargs: Dict[str, Value], dbg: Optional[DebugInfo] = None) -> Value:
        ...
        if isinstance(lhs, NDArrayValue) and (isinstance(rhs, ListValue) or isinstance(rhs, TupleValue)):
            ...
            return builder.op_mat_mul(lhs, rhs, dbg)
            ...
            return NDArrayValue.matmul(
                lhs, rhs,
                expected_dtype,
                lambda x, y: builder.op_add(x, y),
                lambda x, y: builder.op_multiply(x, y),
                lambda: self._build_constant_zero(builder, expected_dtype)
            )
