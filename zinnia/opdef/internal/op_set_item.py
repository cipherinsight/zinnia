...
from zinnia.opdef.abstract.abstract_item_slice import AbstractItemSliceOp
from zinnia.opdef.abstract.abstract_op import AbstractOp
from zinnia.debug.dbg_info import DebugInfo
from zinnia.compile.builder.abstract_ir_builder import AbsIRBuilderInterface
from zinnia.compile.builder.value import Value, NDArrayValue, TupleValue, IntegerValue, ListValue
...
class SetItemOp(AbstractItemSliceOp):
    ...
    def _build_list_assignment(self, builder: AbsIRBuilderInterface, the_self: ListValue, the_value: Value, slicing_param: TupleValue | IntegerValue, dbg: Optional[DebugInfo] = None) -> Value:
        if isinstance(slicing_param, IntegerValue):
            ...
            for i, v in enumerate(the_self.values()):
                new_values[i] = builder.op_select(builder.ir_equal_i(slicing_param, builder.ir_constant_int(i)), the_value, v)
    ...
    def build(self, builder: AbsIRBuilderInterface, kwargs: Dict[str, Value], dbg: Optional[DebugInfo] = None) -> Value:
        ...
        if isinstance(the_self, ListValue):
            ...
            return self._build_list_assignment(builder, the_self, the_value, slicing_param, dbg)
        if isinstance(the_self, NDArrayValue):
            return builder.op_ndarray_set_item(the_self, slicing_params, the_value, dbg)
