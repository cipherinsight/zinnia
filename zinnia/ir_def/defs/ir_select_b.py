...
class SelectBIR(AbstractIR):
    ...
    def infer(self, args: List[Value], dbg: Optional[DebugInfo] = None) -> Any:
        ...
        assert isinstance(cond, BooleanValue) and isinstance(tv, BooleanValue) and isinstance(fv, BooleanValue)
        if cond.val() is None:
            return None
        elif cond.val() != False:
            return tv.val()
    ...
    def build_ir(self, ir_id: int, args: List[Value], dbg: Optional[DebugInfo] = None) -> Tuple[Value, IRStatement]:
        cond, tv, fv = args[0], args[1], args[2]
        assert isinstance(cond, BooleanValue) and isinstance(tv, BooleanValue) and isinstance(fv, BooleanValue)
        return BooleanValue(self.infer(args, dbg), ir_id), IRStatement(ir_id, self, [cond.ptr(), tv.ptr(), fv.ptr()], dbg)

    def export(self) -> Dict:
        return {}

    @staticmethod
    def import_from(data: Dict) -> 'SelectBIR':
        return SelectBIR()
