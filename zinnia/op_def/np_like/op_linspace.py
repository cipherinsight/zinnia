    ...
    def build(self, builder: IRBuilderInterface, kwargs: OpArgsContainer, dbg: Optional[DebugInfo] = None) -> Value:
        ...
        if isinstance(dtype, ClassValue):
            if dtype.val() not in [IntegerType, FloatType]:
                raise TypeInferenceError(dbg, f"Expected `dtype` to be either `int` or `float`, got {dtype.val()}")
        ...
        for i in range(num.val()):
            new_sample = NDArrayValue.binary(start, stop, inferred_dtype, lambda u, v: self._new_sample_op_lambda(
                builder, inferred_dtype, u, v, i, num.val() - (1 if endpoint.val() else 0), dbg
            ))
