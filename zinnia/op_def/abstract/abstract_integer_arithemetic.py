...
from zinnia.compile.triplet import Value, IntegerValue, DynamicNDArrayValue, NDArrayValue
...
class AbstractLogicalArithemetic(AbstractOp):
    ...
    def build_number_and_ndarray(self, builder: IRBuilderInterface, lhs: IntegerValue, rhs: NDArrayValue, dbg: Optional[DebugInfo] = None) -> Value:
        if isinstance(rhs, DynamicNDArrayValue):
            ...
            return dynamic_broadcast_binary(builder, lhs_dyn, rhs, IntegerType, self.get_build_op_lambda(builder), dbg)
    ...
    def build_ndarray_and_number(self, builder: IRBuilderInterface, lhs: NDArrayValue, rhs: IntegerValue, dbg: Optional[DebugInfo] = None) -> Value:
        if isinstance(lhs, DynamicNDArrayValue):
            ...
            return dynamic_broadcast_binary(builder, lhs, rhs_dyn, IntegerType, self.get_build_op_lambda(builder), dbg)
    ...
    def build_ndarray_and_ndarray(self, builder: IRBuilderInterface, lhs: NDArrayValue, rhs: NDArrayValue, dbg: Optional[DebugInfo] = None) -> NDArrayValue:
        if isinstance(lhs, DynamicNDArrayValue) or isinstance(rhs, DynamicNDArrayValue):
            ...
            return dynamic_broadcast_binary(builder, lhs_dyn, rhs_dyn, IntegerType, self.get_build_op_lambda(builder), dbg)
