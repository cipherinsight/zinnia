...
class ImplicitTypeCastOp(AbstractOp):
    ...
    def verify_cast_ability(src: DTDescriptor, dest: DTDescriptor) -> bool:
        ...
        if isinstance(src, TupleDTDescriptor) and isinstance(dest, TupleDTDescriptor):
            if len(src.elements_type) != len(dest.elements_type):
                return False
            for s, d in zip(src.elements_type, dest.elements_type):
    ...
    def recursive_build_implicit_type_cast(self, builder: IRBuilderInterface, src: Value, dest: DTDescriptor, dbg: Optional[DebugInfo] = None) -> Value:
        if src.type() == dest:
            return src
