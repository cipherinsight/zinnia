...
from zinnia.compile.triplet import Value, IntegerValue, NDArrayValue, ListValue, TupleValue, FloatValue
...
class ImplicitTypeAlignOp(AbstractOp):
    ...
    def verify_align_ability(lhs: DTDescriptor, rhs: DTDescriptor) -> bool:
        ...
        if isinstance(lhs, TupleDTDescriptor) and isinstance(rhs, TupleDTDescriptor):
            if len(lhs.elements_type) != len(rhs.elements_type):
                return False
            for l, r in zip(lhs.elements_type, rhs.elements_type):
    ...
    def recursive_build_implicit_type_align(self, builder: IRBuilderInterface, lhs: Value, rhs: Value, dbg: Optional[DebugInfo] = None) -> Tuple[Value, Value]:
        if lhs.type() == rhs.type():
            ...
            ndarray = builder.op_np_asarray(lhs, dbg)
            ...
            ndarray = builder.op_np_asarray(rhs, dbg)
