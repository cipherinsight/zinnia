...
class DynamicNDArray_ZerosOp(AbstractOp):
    def __init__(self):
        super().__init__()

    def get_signature(self) -> str:
        return "DynamicNDArray.zeros"
    ...
    def build(self, builder: IRBuilderInterface, kwargs: OpArgsContainer, dbg: Optional[DebugInfo] = None) -> DynamicNDArrayValue:
        ...
        if parsed_dtype == FloatType:
            values = [builder.ir_constant_float(0.0) for _ in range(bounds.max_length)]
        elif parsed_dtype == IntegerType:
            values = [builder.ir_constant_int(0) for _ in range(bounds.max_length)]
        elif parsed_dtype == BooleanType:
            values = [builder.ir_constant_bool(False) for _ in range(bounds.max_length)]
        else:
